...
MOD = dict(
    ...
    quiz=[
        ...
        dict(q="⟦Đạo hàm đối xứng của hàm dốc $R$ tại 0 (với $H(0)=\\tfrac12$) bằng bao nhiêu?||What is the symmetric derivative of the ramp $R$ at 0 (with $H(0)=\\tfrac12$)?⟧",
             opts=["{{r_prime0}}", "1", "0", "2"], explain="⟦$[R(\\Delta)-R(-\\Delta)]/2\\Delta=\\tfrac12$ = {{r_prime0}}, đúng giá trị giữa.||$[R(\\Delta)-R(-\\Delta)]/2\\Delta=\\tfrac12$ = {{r_prime0}}, the midvalue.⟧"),
        ...
        dict(q="⟦Sau khi lọc bằng sinc cắt cụt, độ lợi tại tần số 0.8 (trên tần số cắt 0.5) gần bằng bao nhiêu?||After filtering with a truncated sinc, what is the gain at frequency 0.8 (above the 0.5 cutoff), approximately?⟧",
             opts=["{{lp_gain_08}}", "1", "0.5", "0.2"], explain="⟦Sinc lý tưởng bỏ hoàn toàn 0.8; bản cắt cụt còn {{lp_gain_08}}, và giữ {{lp_gain_02}} tại 0.2.||An ideal sinc removes 0.8 entirely; the truncated version leaves {{lp_gain_08}}, and keeps {{lp_gain_02}} at 0.2.⟧"),
        ...
        dict(q="⟦Vì sao Bracewell chọn $e^{-\\pi x^2}$ làm chuẩn cho hàm Gauss?||Why does Bracewell choose $e^{-\\pi x^2}$ as the standard Gaussian?⟧",
             opts=["⟦Tung độ trung tâm và diện tích đều bằng 1, và biến đổi của nó chuẩn hóa y hệt||Its central ordinate and area are both 1, and its transform is normalised identically⟧",
